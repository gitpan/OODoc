...
They are completed with the defaults set by L<createManual(format_options)|OODoc::Format/"Attributes">.
...
called with all the options that L<showChapter()|OODoc::Format/"Attributes"> usually gets.
...
See L<formatManual(append)|"METHODS"> for an explanation.
...
L<createManual(format_options)|OODoc::Format/"Attributes"> or L<OODoc::create(format_options)|OODoc/"Formatter">
can be used with a list of formatting options which are passed to
L<showChapter()|OODoc::Format/"Attributes">.  This will help you to produce pages which have
