...
$obj-E<gt>B<processFiles>(OPTIONS)
...
$obj-E<gt>B<selectFiles>(WHICH, LIST)
...
L<processFiles(select)|"Parser"> and the LIST are files from a manifest.
...
$obj-E<gt>B<getPackageRelations>
...
of the packages.  C<prepare> must be called between L<processFiles()|"Parser">
and L<create()|"Formatter">.
...
$obj-E<gt>B<create>(NAME|CLASS|OBJECT, OPTIONS)
...
The value is passed on to L<OODoc::Format::createManual(append)|OODoc::Format/"Attributes">,
...
Options passed to L<OODoc::Format::createManual(format_options)|OODoc::Format/"Attributes"> when
...
Passed to L<OODoc::Format::createManual(template)|OODoc::Format/"Attributes">, and defines the
...
passed to L<OODoc::Format::createOtherPages(source)|OODoc::Format/"Attributes">.
...
passed to L<OODoc::Format::createOtherPages(process)|OODoc::Format/"Attributes">.
...
I<Error:> Cannot compile $parser class

The $parser class does not exist or produces compiler errors.

I<Error:> Parser $parser could not be instantiated
...
I<Warning:> no file $fn to include in the distribution
...
I<Error:> no version specified for module "$name"
...
I<Error:> problems compiling $code for package $name: $@
...
I<Error:> requires a directory to write the distribution to
...
I<Error:> requires a directory to write the manuals to
...
I<Error:> the produced module needs a descriptive name
...
L<OODoc::Parser::Markov|OODoc::Parser::Markov> for its syntax.  The formatter is
described in L<OODoc::Format::Pod|OODoc::Format::Pod>.
