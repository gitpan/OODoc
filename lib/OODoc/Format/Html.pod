...
=item $obj-E<gt>B<templateDate>(ZONE, ARGS)

=item $obj-E<gt>B<templateDistribution>(ZONE, ARGS)

The name of the distribution which contains the manual page at hand.

=item $obj-E<gt>B<templateHref>(ZONE, ARGS)

=item $obj-E<gt>B<templateIndex>(ZONE, ARGS)
...
=item $obj-E<gt>B<templateInheritance>(ZONE, ARGS)

=item $obj-E<gt>B<templateList>(ZONE, ARGS)
...
=item $obj-E<gt>B<templateManual>(ZONE, ARGS)

=item $obj-E<gt>B<templateMeta>(ZONE, ARGS)
...
=item $obj-E<gt>B<templateName>(ZONE, ARGS)

=item $obj-E<gt>B<templateTitle>(ZONE, ARGS)

=item $obj-E<gt>B<templateVersion>(ZONE, ARGS)
...
=item Error: cannot write html manual at $filename: $!
...
=item Error: html source directory $source does not exist.
...
=item Error: no directory to put other html pages in.
...
=item Warning: no meaning for container $container in index block

=item Error: no package name for html production
